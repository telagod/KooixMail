...
📎 **附件支持** — SMTP 收信自动解析 MIME 附件，持久化存储，浏览器一键下载

⚡ **实时推送** — 新邮件到达时浏览器即时通知，断线自动重连，支持多实例部署

🛡️ **反垃圾邮件** — 内置 SPF/DKIM/DMARC 校验、速率限制、灰名单、黑名单查询（重启不丢数据）
...
| 下载附件 | `GET /messages/:id/attachments/:aid` |
...
后端 99 个测试覆盖了认证、数据库、全部 API 路由、邮件投递策略、SMTP 协议、附件持久化和事件系统。
...
- 📤 不支持发送邮件（仅收信）
- 📎 HTTP 投递接口暂不支持附件上传（SMTP 收信支持附件）
- 🔄 速率限制/灰名单每 60 秒刷盘一次，极端情况下 crash 可能丢失最近 60 秒数据
